* Что происходит при прерывании скрипта `text-trap.sh`? Объясните, почему.
    ```
    При нажатии на комбинацию клавиш Ctrl+c во время выполнения сработает реакция на сигнал SIGINT (Signal Interrupt), установленная командой trap, и программа аварийно завершится.
    ```
...
* Что означает `pos` при выводе содержимого файла `/proc/$$/fdinfo/3`?
    ```
    Текущую позицию указателя чтения-записи в открытый файл 3 процесса оболочки Bash.
    ```
